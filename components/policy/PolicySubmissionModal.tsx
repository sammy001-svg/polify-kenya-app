...
import { PolifyAIAnalyst } from "./PolifyAIAnalyst";
...
export function PolicySubmissionModal({
  isOpen,
  onClose,
  onSuccess,
}: PolicySubmissionModalProps) {
  ...
  const handleSubmitFinal = async () => {
    ...
    const { error } = await supabase.from("policy_ideas").insert({
          ...
          {step === 1 && (
            <div className="space-y-6 animate-in slide-in-from-right-4 duration-500">
              ...
              <Button
                ...
                className="w-full bg-brand-primary h-12 font-bold rounded-full group"
              >
                Next Step{" "}
            ...
            <div className="space-y-6 animate-in zoom-in-95 duration-500 text-center">
              ...
              <div className="grid grid-cols-3 gap-4">
                <div className="space-y-2 p-4 bg-brand-surface-secondary rounded-2xl border border-border">
                  <p className="text-[10px] font-black uppercase tracking-tighter text-brand-text-muted">
                    Feasibility
                  </p>
                ...
                <div className="space-y-2 p-4 bg-brand-surface-secondary rounded-2xl border border-border">
                  ...
                  <p className="text-2xl font-black text-kenya-red">
                    {analysis.cost_index}/100
                  </p>
                </div>
                <div className="space-y-2 p-4 bg-brand-surface-secondary rounded-2xl border border-border">
                  <p className="text-[10px] font-black uppercase tracking-tighter text-brand-text-muted">
                  ...
                  <p className="text-2xl font-black text-kenya-green">
              ...
              <div className="bg-brand-surface-secondary/50 p-6 rounded-2xl border border-dashed border-border text-left relative">
                ...
                <h4 className="text-xs font-bold uppercase tracking-widest text-brand-primary mb-2 flex items-center gap-2">
                  <CheckCircle2 className="w-4 h-4" /> Polify Analyst Note
                </h4>
