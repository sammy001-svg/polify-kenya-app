/* cSpell:ignore Polify analyst */
"use client";

import { useState, useEffect } from "react";
import { Cpu, Binary, BarChart3, Wallet, Users } from "lucide-react";
...
interface PolifyAIAnalystProps {
  onComplete: (analysis: AnalysisResult) => void;
}

export function PolifyAIAnalyst({ onComplete }: PolifyAIAnalystProps) {
  ...
  const steps = [
    ...
    { label: "Finalizing Polify Report", icon: BarChart3 },
  ];
  ...
  useEffect(() => {
    ...
    const completeTimeout = setTimeout(() => {
      const simulatedAnalysis: AnalysisResult = {
        ...
        analyst_notes:
          "This proposal shows high alignment with the Kenya Vision 2030 social pillar. Significant fiscal space would be required, but long-term ROI is projected to be positive.",
      };
    ...
    <div className="flex flex-col items-center justify-center space-y-8 py-10 animate-in fade-in zoom-in-95 duration-700">
      ...
      <div className="text-center space-y-4 max-w-sm w-full">
        ...
        <div className="space-y-2">
          ...
          <Progress
            value={progress}
            className="h-1 bg-brand-surface-highlight"
          />
        </div>
        <p className="text-xs text-brand-text-muted leading-relaxed italic">
          &quot;We are reviewing your proposal against current legislative
          frameworks and fiscal capacity...&quot;
        </p>
      </div>
