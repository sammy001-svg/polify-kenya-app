...
export function BillKanban({ bills, onVote }: BillKanbanProps) {
  ...
  const billsByStage = STAGES.reduce((acc, stage) => {
            ...
            return (
                <div key={stage} className="w-[85vw] sm:w-[300px] shrink-0 flex flex-col h-[500px] sm:h-[600px] bg-brand-surface/30 rounded-2xl border border-white/5 overflow-hidden snap-center">
                  ...
                  <div className={cn(
                    ...
                    stage === 'Presidential Assent' && "bg-kenya-green/10 border-kenya-green/30"
                  )}>
                     <div className="flex items-center gap-2">
                        ...
                        <h3 className="font-bold text-sm text-white uppercase tracking-tight">{stage}</h3>
                     </div>
