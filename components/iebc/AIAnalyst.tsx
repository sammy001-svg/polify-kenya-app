...
export function AIAnalyst() {
  const [messages, setMessages] = React.useState<Message[]>([
    { role: 'ai', content: "Jambo! I am the IEBC AI Analyst. I can help you analyze electoral data or answer questions about the IEBC. Try clicking 'Analyze Live Data' or ask me a question!" }
  ]);
  ...
  const handleAnalyze = async () => {
    setIsAnalyzing(true);
    setMessages(prev => [...prev, { role: 'user', content: "Analyze the current live data." }]);
    
    try {
      const response = await analyzeIEBCData();
      setMessages(prev => [...prev, { role: 'ai', content: response.text, type: 'analysis' }]);
    } catch {
      setMessages(prev => [...prev, { role: 'ai', content: "Sorry, I encountered an error analyzing the data." }]);
    } finally {
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'ai', content: response.text, type: 'answer' }]);
    } catch {
        setMessages(prev => [...prev, { role: 'ai', content: "Sorry, I couldn't process your request." }]);
    } finally {
      setIsAnalyzing(false);
    }
  };
