...
import { MapPin, User, Star, ShoppingCart, Check } from "lucide-react";
...
export function MarketplaceCard({ item }: MarketplaceCardProps) {
  ...
  return (
    <Link href={`/marketplace/${item.id}`} className="group h-full block">
      <Card className="overflow-hidden bg-kenya-deep border-kenya-green/20 hover:border-kenya-green/50 transition-all duration-300 flex flex-col h-full hover:shadow-xl">
          ...
          <Button
            ...
            className={`w-full transition-all font-bold ${
              isAdded 
                ...
                : "bg-white/5 hover:bg-brand-primary hover:text-black text-white border border-white/10 group-hover:border-brand-primary/50"
            }`}
          >
            {isAdded ? (
