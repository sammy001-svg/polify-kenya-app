...
import { Search, ShieldCheck, FileText, ExternalLink, Image as ImageIcon, Link2 } from "lucide-react";
...
export function AuditVault() {
    ...
    return (
        <div className="bg-black/40 border border-white/10 rounded-xl overflow-hidden backdrop-blur-md h-[600px] flex flex-col">
            {/* Header */}
            <div className="p-4 border-b border-white/10 bg-white/5 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                <div>
                    <h3 className="text-lg font-black text-white flex items-center gap-2">
                        <ShieldCheck className="w-5 h-5 text-kenya-green" />
                        DIGITAL AUDIT VAULT
                    </h3>
                    <p className="text-xs text-gray-500 font-mono">VERIFY DIGITAL TALLIES AGAINST SOURCE FORMS</p>
                </div>
                
                <div className="flex w-full md:w-auto gap-2">
                    <Input 
                        placeholder="Search Polling Station... (e.g. Kibra)" 
                        className="bg-black/50 border-white/10 text-xs text-white h-9"
                        value={query}
                    ...
                    <Button 
                        size="sm" 
                        className="bg-kenya-green hover:bg-kenya-green/80 text-white h-9"
                        ...
                        <div className="space-y-2">
                            {results.length === 0 && !searching && (
                                <div className="text-center py-10 opacity-30">
                                    ...
                                    <p className="text-xs">No records found. Search by name or ID.</p>
                                </div>
                            )}
                            {results.map((station) => (
                                <button
                                ...
                                >
                                    <div className="flex justify-between items-start mb-1">
                                        <Badge variant="outline" className="text-[8px] h-3 border-white/20 text-gray-400">
                                            {station.station_id}
                                        </Badge>
                                    ...
                                    <div className="text-sm font-bold text-white truncate">{station.station_name}</div>
                                    <div className="text-[10px] text-gray-500 truncate">{station.location}</div>
                                </button>
                            ))}
                        </div>
                    </ScrollArea>
                ...
                <div className="hidden md:flex flex-1 flex-col bg-black/60">
                    {selectedStation ? (
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="aspect-[3/4] bg-white/5 rounded-lg border border-white/10 overflow-hidden relative group">
                                        ...
                                        <div className="absolute inset-0 flex items-center justify-center bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity">
                                            <Button variant="outline" size="sm" className="bg-black text-white border-white/20">
                                    ...
                                    <div className="p-3 bg-red-900/10 border border-red-500/20 rounded-lg">
                                        <p className="text-[10px] text-red-100/60 leading-relaxed italic">
                                            * This is a high-fidelity simulation for audit demonstration. Original documents are sourced from the IEBC Portal.
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-6">
                                    <div className="space-y-4">
                                        ...
                                        <div className="bg-black/80 border border-white/10 rounded-xl p-4 space-y-4">
                                            {Object.entries(selectedStation.digital_tally).sort((a,b) => b[1]-a[1]).map(([candidate, votes]) => (
                                                <div key={candidate} className="space-y-1">
                                                    <div className="flex justify-between text-xs">
                                                        <span className="text-gray-400">{candidate}</span>
                                                        <span className="text-white font-bold">{votes.toLocaleString()} votes</span>
                                                    </div>
                                                    <div className="h-1.5 w-full bg-white/5 rounded-full overflow-hidden">
                                                        <div className="h-full bg-kenya-green/50" style={{ width: `${(votes / 2100) * 100}%` }} />
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center opacity-20">
                            ...
                            <p className="text-sm font-black tracking-widest uppercase">Select a station to audit</p>
                        </div>
                    )}
                </div>
            </div>
