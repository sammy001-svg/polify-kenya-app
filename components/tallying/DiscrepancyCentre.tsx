...
import { ConflictingResult, getConflictingResults, resolveDiscrepancy } from "@/actions/tallying";
import { AlertCircle, CheckCircle2, Gavel, ArrowRight, History, ShieldAlert, MessageSquare, Save } from "lucide-react";
...
export function DiscrepancyCentre() {
    ...
    return (
        <div className="bg-black/40 border border-white/10 rounded-xl overflow-hidden backdrop-blur-md flex flex-col h-[700px]">
            {/* Header */}
            <div className="p-4 border-b border-white/10 bg-white/5 flex justify-between items-center">
                <div>
                    <h3 className="text-lg font-black text-white flex items-center gap-2">
                        <Gavel className="w-5 h-5 text-kenya-red" />
                        DISCREPANCY CONTROL CENTRE
                    </h3>
                    <p className="text-xs text-gray-500 font-mono">HUMAN-IN-THE-LOOP CONFLICT RESOLUTION</p>
                </div>
                <Badge variant="destructive" className="animate-pulse bg-red-900/50 text-red-500 border-red-500/50">
                            ...
                            {conflicts.map((conflict) => (
                                ...
                                >
                                    <div className="flex justify-between items-start mb-1">
                                        <Badge variant="outline" className={`text-[8px] h-3 ${conflict.status === 'PENDING' ? 'border-red-500/50 text-red-400' : 'border-green-500/50 text-green-400'}`}>
                                            {conflict.status}
                                        </Badge>
                                        <span className="text-[10px] text-gray-500 font-mono">#{conflict.id}</span>
                                    </div>
                                    <div className="text-sm font-bold text-white truncate">{conflict.station_name}</div>
                ...
                <div className="flex-1 flex flex-col bg-black/60 relative">
                    {selected ? (
                            ...
                            <div className="p-6 grid grid-cols-1 lg:grid-cols-2 gap-8 border-b border-white/10">
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <Badge variant="outline" className="text-[8px] border-red-500/30 text-red-400">UNTRUSTED</Badge>
                                    </div>
                                    <div className="bg-red-900/10 border border-red-500/20 rounded-xl p-4 space-y-4">
                                        {Object.entries(selected.media_value).map(([candidate, votes]) => (
                                            <div key={candidate} className="flex justify-between items-center bg-black/40 p-2 rounded-lg border border-red-500/10">
                                                <span className="text-xs text-gray-400">{candidate}</span>
                                                <span className="text-xs font-bold text-red-400">{votes.toLocaleString()}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <Badge variant="outline" className="text-[8px] border-green-500/30 text-green-400">VERIFIED REFERENCE</Badge>
                                    </div>
                                    <div className="bg-kenya-green/10 border border-kenya-green/20 rounded-xl p-4 space-y-4">
                                        {Object.entries(selected.iebc_value).map(([candidate, votes]) => (
                                            <div key={candidate} className="flex justify-between items-center bg-black/40 p-2 rounded-lg border border-green-500/10">
                                                <span className="text-xs text-gray-400">{candidate}</span>
                                                <span className="text-xs font-bold text-kenya-green">{votes.toLocaleString()}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                            ...
                            <div className="flex-1 p-6 space-y-6">
                                <div className="space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                                        ...
                                        <div className="space-y-3">
                                            <p className="text-[10px] text-gray-500 font-bold uppercase">Final Tally Correction</p>
                                            {Object.keys(selected.iebc_value).map((candidate) => (
                                                <div key={candidate} className="flex items-center gap-3">
                                                    <span className="text-[10px] text-gray-400 w-32 truncate">{candidate}</span>
                                                    <Input 
                                                        type="number"
                                                        value={corrections[candidate] || 0}
                                                        onChange={(e) => setCorrections({ ...corrections, [candidate]: parseInt(e.target.value) })}
                                        ...
                                        {/* Observer Rationale */}
                                        <div className="space-y-3">
                                            <p className="text-[10px] text-gray-500 font-bold uppercase">Audit Rationale (Required)</p>
                                            <Textarea 
                                ...
                                <div className="pt-4 border-t border-white/10 flex justify-between items-center">
                                    <div className="flex items-center gap-2 text-[10px] text-gray-500">
                                        ...
                                        Resolution will be hashed and committed to Global Audit Vault
                                    </div>
                                    <Button 
                        ...
                        <div className="flex-1 flex flex-col items-center justify-center opacity-20">
                            ...
                            <p className="text-sm font-black tracking-widest uppercase">Select a conflict to resolve</p>
                        </div>
                    )}
